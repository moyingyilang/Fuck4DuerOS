...
**第十七条**　处理个人信息前应当以显著方式、清晰易懂的语言真实、准确、完整地向个人告知。
...
本报告记录在小度学生手机设备中发现的一套完整 PCDN（P2P Content Delivery Network，点对点内容分发网络）客户端及其配套监控系统。该组件具有以下特征：
...
该 SDK 属于商业级 APM（Application Performance Monitoring）系统，采集范围覆盖应用行为、网络、电量、崩溃等多个维度。
